use pciid_parser::{Database, schema::SubDeviceId};
use std::{
...
fn main() {
    ...
    let Some(vendor) = db.vendors.get("8086") else {
        return;
    };
    vendor.devices.iter().for_each(|(device_id, device)| {
        println!("device_id: {} device_name: {}", device_id, device.name);
    });
    let devices = get_pci_devices().unwrap();
    for device in devices {
        device.get_device_name(&db).map(|name| {
            println!("name: {} type: {}", name, device.read_value(PciDeviceReadableValues::Vendor));
        });
    }
}
